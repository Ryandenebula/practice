...
file_path = "新五种数据集/5%噪声5%丢失改变误差/误差30%/train.txt"  # 请替换为实际的六种雷达数据文件路径
...
class MambaClassifier(nn.Module):
    def __init__(self, input_dim=3, hidden_dim=128, num_classes=6):
...
def load_and_preprocess_data(file_path):
    ...
    if not np.all(np.isin(unique_labels, [0, 1, 2, 3, 4, 5])):
