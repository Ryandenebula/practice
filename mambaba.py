...
# python Mamba_new.py
file_path = "新8种数据集/5%误差5%丢失5%噪声 参数调整数据集/train.txt"  # 请替换为实际的六种雷达数据文件路径
...
def main():
    ...
    cosine_steps  = (100 - warmup_epochs) * len(train_loader)
...
# 构造组合调度器
    ...
    print("开始训练...")
    model = train_model(model, train_loader, test_loader, criterion, optimizer, scheduler, num_epochs=100)
