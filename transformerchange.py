...
## 数据集加载
ori_traindata = np.loadtxt("新8种数据集/5%误差5%噪声改变丢失/丢失率20%/train.txt", delimiter=',', usecols=(0, 1, 2))
ori_traindata = torch.from_numpy(ori_traindata).float()
ori_traindata = ori_traindata.reshape(-1, 1000, 3)

ori_labels = np.loadtxt("新8种数据集/5%误差5%噪声改变丢失/丢失率20%/train.txt", delimiter=',', usecols=(3,))
train_label = torch.from_numpy(ori_labels).long()
train_label = train_label.view(-1, 1000)

ori_testdata = np.loadtxt("新8种数据集/5%误差5%噪声改变丢失/丢失率20%/train.txt", delimiter=',', usecols=(0, 1, 2))
ori_testdata = torch.from_numpy(ori_testdata).float()
ori_testdata = ori_testdata.view(-1, 1000, 3)

ori_testlabels = np.loadtxt("新8种数据集/5%误差5%噪声改变丢失/丢失率20%/train.txt", delimiter=',', usecols=(3,))
